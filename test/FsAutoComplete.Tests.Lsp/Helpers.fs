...
let createServer (state: State) =
  ...
  let client =
    FSharpLspClient(
      ...
      { new Ionide.LanguageServerProtocol.Server.ClientRequestSender with
          member __.Send _ _ = AsyncLspResult.notImplemented }
    )
...
let logDotnetRestore section line =
  if not (String.IsNullOrWhiteSpace(line)) then
    logger.debug (
      eventX "[{section}] dotnet restore: {line}"
...
let parseProject projectFilePath (server: FSharpLspServer) =
  async {
    ...
    logger.debug (
      eventX "{project} parse result: {result}"
      >> setField "result" (sprintf "%A" result)
      >> setField "project" projectName
    )
  }
...
let private matchFiles (files: string Set) =
  Observable.choose (fun (p: Ionide.LanguageServerProtocol.Types.PublishDiagnosticsParams) ->
    let filename =
      p.Uri.Split([| '/'; '\\' |], StringSplitOptions.RemoveEmptyEntries)
      |> Array.last
...
let fileDiagnostics file =
  logger.info (
    eventX "waiting for events on file {file}"
    >> setField "file" file
  )
