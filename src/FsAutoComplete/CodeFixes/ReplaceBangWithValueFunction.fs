...
let fix (getParseResultsForFile: GetParseResultsForFile) (getLineText: GetLineText) : CodeFix =
  fun codeActionParams ->
    asyncResult {
      ...
      let selectionRange =
        protocolRangeToRange (codeActionParams.TextDocument.GetFilePath()) codeActionParams.Range

      let! parseResults, line, lines = getParseResultsForFile fileName selectionRange.Start

      let! derefRange =
        parseResults.GetParseResults.TryRangeOfRefCellDereferenceContainingPos selectionRange.Start
        |> Result.ofOption (fun _ -> "No deref found at that pos")

      let! exprRange =
        parseResults.GetParseResults.TryRangeOfExpressionBeingDereferencedContainingPos selectionRange.Start
        |> Result.ofOption (fun _ -> "No expr found at that pos")
