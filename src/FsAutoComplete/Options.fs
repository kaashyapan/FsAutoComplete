...
module Options =
  ...
  let apply
    ...
    (logConfig: Serilog.LoggerConfiguration)
    ...
    let applyArg arg =
      ...
      | Logfile s ->
        try
          logConfig.WriteTo.Async (fun c ->
            c.File(path = s, levelSwitch = levelSwitch)
            |> ignore)
          |> ignore
        with
        | e ->
      ...
      | Filter categories ->
        ...
        |> List.iter (fun category ->
          ...
          logConfig.Filter.ByExcluding(Func<_, _>(isCategory category))
          |> ignore)
      | Version
      | WaitForDebugger
    ...
    args.GetAllResults() |> List.iter applyArg
