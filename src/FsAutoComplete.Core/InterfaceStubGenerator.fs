...
let private walkTypeDefn pos (SynTypeDefn (members = members; implicitConstructor = implicitCtor)) =
  ...
  |> List.tryPick (fun m ->
    if Range.rangeContainsPos m.Range pos then
      match m with
  ...
  SyntaxTraversal.Traverse(
    ...
    { new SyntaxVisitorBase<_>() with
        ...
        override _.VisitModuleDecl(_, defaultTraverse, decl) =
          match decl with
          | SynModuleDecl.Types (types, _) -> List.tryPick (walkTypeDefn pos) types
  ...
  async {
          ...
          | Some symbolUse ->
            ...
            | :? FSharpEntity as entity ->
              if isInterface entity then
                ...
                | Some displayContext -> Some(interfaceData, displayContext, entity)
                | None -> None
              else
                None
            | _ -> None
    ...
    | Some (interfaceData, displayContext, entity) ->
      ...
      let generatedString =
        ...
        match interfaceData with
        | InterfaceData.Interface _ -> formattedString
        | InterfaceData.ObjExpr _ -> formattedString.TrimEnd('\n')
