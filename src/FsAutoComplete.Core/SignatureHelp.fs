...
let private charAt (lines: ISourceText) (pos: Position) = (lineText lines pos).[pos.Column - 1]
  ...
  asyncMaybe {
    ...
    | :? FSharpMemberOrFunctionOrValue as mfv when isValid mfv ->
      ...
      | _ ->
        ...
        let methods =
          tyRes.GetCheckResults.GetMethods(
            symbolStart.Line,
            symbolUse.Range.EndColumn,
            lineText lines symbolStart,
            None
          )
