...
let scriptRoot = @"..\test\FsAutoComplete.Tests.Lsp\TestCases"
let testGroup = "CompletionAutoOpenTests"
let testCase = "ModuleDocsAndNewLineBeforeOpen.fsx"

let fullPath =
  Path.Combine(scriptRoot, testGroup, testCase)
  |> Path.GetFullPath

let projectOpts, projectOptsErrors = getNetCoreScriptOptions fullPath
let parseResult, checkResult = parseAndCheckFile (fullPath, projectOpts)
let pErrors = parseResult.Diagnostics
let cErrors = checkResult.Value.Diagnostics
